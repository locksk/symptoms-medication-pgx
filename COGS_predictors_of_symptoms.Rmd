...
#### preparing medication variables ####
...
# remove non antipsychotic drugs
pp$Antipsychotic[!(pp$Antipsychotic %in% AP_codes)] <- NA
pp$antidepressant[!(pp$antidepressant %in% AP_codes)] <- NA
pp$Mood_Stabiliser[!(pp$Mood_Stabiliser%in% AP_codes)] <- NA
pp$Benzodiazepine[!(pp$Benzodiazepine %in% AP_codes)] <- NA
pp$Anticholinergic[!(pp$Anticholinergic %in% AP_codes)] <- NA
pp$other_medication[!(pp$other_medication %in% AP_codes)] <- NA

# get medication column names
cols <- colnames(pp[-1])
...
# convert numbers to drug names
pp_freq <- as.data.frame(recode_selected_codes(df = pp, recode_vector = recode_vector, columns_to_recode = cols))

# create variable with all medication names per row in it, separated by commas, no duplicates, and in alphabetical order
pp_freq <- pp_freq %>%
  mutate(total = pmap_chr(dplyr::select(., -Study_ID), ~toString(sort(unique(na.omit(c(...)))))))

# identify people on APP vs not APP by whether or not there is a comma in the variable
pp_freq <- pp_freq %>%
  mutate(ap_poly = case_when(grepl(",", total) ~ 1,
                            !grepl(",", total) ~0))

pp <- dplyr::select(pp_freq, c("Study_ID", "ap_poly"))
...
This study followed the STrengthening the REporting of Genetic Association Studies reporting recommendations^45^ (STREGA), an extension of the STROBE Statement^46^. The checklist can be found <a href=Writing/figs/STREGA.pdf target="_blank">here</a>. 
